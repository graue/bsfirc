...
static void *smalloc(char *fname, int lineno, size_t nbytes)
{
	char *buf;
...
void *sm_malloc(char *fname, int lineno, size_t nbytes)
{
	void *buf;
...
void *sm_calloc(char *fname, int lineno, size_t nelem, size_t elsize)
{
	void *buf;
...
void *sm_realloc(char *fname, int lineno, void *ptr, size_t size)
{
	size_t osize;
	void *buf;
...
char *sm_strdup(char *fname, int lineno, const char *s)
{
	char *scopy;
	int slen = strlen(s);

	scopy = smalloc(fname, lineno, slen+1);
	if (scopy != NULL)
...
/*  ACTUALLYSTRDUP  --  Interface to system strdup() function. */
char *actuallystrdup(const char *s)
{
	return strdup(s);
}
